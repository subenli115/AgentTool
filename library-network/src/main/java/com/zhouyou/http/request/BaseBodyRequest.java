...
import com.zhouyou.http.utils.EncodeUtil;
...
public abstract class BaseBodyRequest<R extends BaseBodyRequest> extends BaseRequest<R> {
    ...
    public R headerClientId() {
        this.headers("client_id", "app");
        return (R) this;
    }

    public R headerClientSecret() {
        this.headers("client_secret", "123456");
        return (R) this;
    }

    public R headerClientIdClientSecret() {
        headerClientId();
        headerClientSecret();
        return (R) this;
    }
    ...
    /**
     * 加入json参数，
     * 最后需要调用upJson转化成json字符串
     *
     * @param key   参数名
     * @param value 参数值
     * @return
     * @usage: 使用方法
     * * jsonParam("feedbackId": "1")
     * * jsonParam("replyContent": "abc")
     * * 转化后形如：
     * * {"feedbackId": "1", "replyContent": "abc"}
     */
    public R jsonParam(String key, Object value) {
        map.put(key, value);
        return (R) this;
    }
    ...
    public R upJson(@Body Object object) {
    ...
    protected Observable<ResponseBody> uploadFilesWithParts() {
        ...
        for (Map.Entry<String, List<HttpParams.FileWrapper>> entry : params.fileParamsMap.entrySet()) {
            ...
            for (HttpParams.FileWrapper fileWrapper : fileValues) {
                ...
                part = addFile(entry.getKey(), fileWrapper);
                parts.add(part);
            }
        }
    ...
    private MultipartBody.Part addFile(String key, HttpParams.FileWrapper fileWrapper) {
        ...
        if (fileWrapper.responseCallBack != null) {
            UploadProgressRequestBody uploadProgressRequestBody = new UploadProgressRequestBody(requestBody, fileWrapper.responseCallBack);
            MultipartBody.Part part = null;
            part = MultipartBody.Part.createFormData(key, EncodeUtil.encodeUTF(fileWrapper.fileName), uploadProgressRequestBody);
            return part;
        } else {
            MultipartBody.Part part = MultipartBody.Part.createFormData(key, EncodeUtil.encodeUTF(fileWrapper.fileName), requestBody);
            return part;
        }
    }
