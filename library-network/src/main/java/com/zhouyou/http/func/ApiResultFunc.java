...
public class ApiResultFunc<T> implements Function<ResponseBody, ApiResult<T>> {
                    ...
                    if (!List.class.isAssignableFrom(rawType) && clazz.equals(String.class)) {
                       ...
                       /* final Type type = Utils.getType(cls, 0);
                        ...
                        if(jsonObject.has("code")){
                            ...
                            if(codeElement.getAsInt()==401006){
                                sendBroadcast();
                                apiResult.setMsg("身份认证令牌无效");
                                return  apiResult;
                            }else {
                            }
                        ...
                        if (jsonObject.has("data")) {
                            ...
                            if (dataElement == null //什么都没有
                                    || "null".equals(dataElement.toString())//"null"
                                    || "\"\"".equals(dataElement.toString())//""
                                    || TextUtils.isEmpty(dataElement.toString())) {
                                jsonObject.addProperty("data", clazz.equals(String.class) ? "" : null);
                            }
                            ApiResult result = gson.fromJson(json, type);
                            if (result != null) {
                                apiResult = result;
    ...
    private void sendBroadcast() {
        LocalBroadcastManager.getInstance(EasyHttp.getContext()).sendBroadcast(new Intent("api.result.code.401006"));
    }
